...
db_conn = mysql.connector.connect(
    host="localhost", user="pyadmin", password="qzq", database="jzon_serv"
)
...
def new_car_form():
    ...
    # get the last inserted ID and the next auto-increment ID, 
    # to ensure the next car record will have an id value of last_id + 1, 
    # otherwise use a separate sql query to specify the ID, to provide ID continuity
    qurzor.execute("select id from carz order by id desc limit 1")
    last_id = qurzor.fetchone()
    last_id = last_id[0]

    qurzor.execute(
        'select auto_increment from information_schema.tables where table_schema = "jzon_serv" and table_name = "carz"'
    )
