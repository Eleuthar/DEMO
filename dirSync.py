...
class DirSync:
	...
	def remove_old_copies(self, ndx_on_dst: list[int], cloud_hexmap: dict[str, list], logger: IO) -> int:
		diff_counter = 0
	...
	def handle_duplicates(self, dst_hex, client_hexmap, cloud_hexmap, logger) -> int:
		...
		# one of the lists is bigger by at least 1 and the other is now empty
			...
			if len(ndx_on_dst) == 0:
				diff_counter = self.dump_client_copies(ndx_on_src, client_hexmap, logger)
		...
		for j in range(len(cloud_hexmap["hex"])):
			...
			if dst_hex in client_hexmap["hex"]:
				...
				if client_hexmap["hex"].count(dst_hex) > 1 or cloud_hexmap["hex"].count(dst_hex) > 1:
					...
					diff_counter += self.handle_duplicates(dst_hex, client_hexmap, cloud_hexmap, logger)
					continue
